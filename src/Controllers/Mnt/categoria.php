...
class categoria extends PublicController{
    ...
    private function yeah($message){
        \Utilities\Site::redirectToWithMsg("index.php?page=mnt_categorias",$message);
    }
    public function run():void{
        ...
        if($this->isPostBack()){
            ...
            if(($viewData["catest"]=="INA" || $viewData["catest"]=="ACT" || $viewData["catest"]=="PLN") ==false){
                $viewData["hasErrors"]=true;
            ...
            if(!$viewData["hasErrors"]){
                ...
                switch($viewData["mode"])
                {
                    case "INS":
                            ...
                            {
                                $this->yeah("Categoria Creada");
                            }
                        break;

                    case "UPD":
                        if(\Dao\Mnt\Categorias::editarCategoria( $viewData["catnom"],$viewData["catid"],$viewData["catid"]))
                            {
                                $this->yeah("Categoria Actualizada");
                            }
        ...
        {
            $tmpCategoria = \Dao\Mnt\Categorias::obtenerUnicaCategoria($viewData["catid"]);
            /*
            ...
            */
            ...
            $viewData["mode_dsc"] = sprintf($modeDscArr[$viewData["mode"]],$viewData["catid"],$viewData["catnom"]);
        ...
        //Generar un token XSRF para evitar ataques
        $viewData["xsrftoken"] = md5("categoria".random_int(10000,99999));
